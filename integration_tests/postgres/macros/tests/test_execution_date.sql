{% macro test_execution_date() %}
  ...
  {% set expected = modules.datetime.datetime.utcnow().isoformat() %}
  {% set actual = dbt_airflow_macros.execution_date() %}
  {{ dbt_unittest.assert_equals(expected, actual, 'test_execution_date_utcnow_no_timezone') }}

  {# Test case 2: EXECUTION_DATE not set, with timezone #}
  {% set expected_dt = modules.datetime.datetime.utcnow() %}
  {% set expected = modules.pytz.timezone("Asia/Tokyo").localize(expected_dt).isoformat() %}
  {% set actual = dbt_airflow_macros.execution_date(timezone="Asia/Tokyo") %}
  {{ dbt_unittest.assert_equals(expected, actual, 'test_execution_date_utcnow_with_timezone') }}
  ...
  {% set expected = modules.pytz.timezone("America/New_York").localize(expected_dt).isoformat() %}
  ...
  {% set env_var_val = "2023-01-15T10:30:00+09:00" %}
  {% set expected_dt = modules.datetime.datetime.fromisoformat(env_var_val) %}
  {% set expected = modules.pytz.timezone("Europe/London").localize(expected_dt).isoformat() %}
